  ...
  module Adapter
    ...
    class Windows < Base
      ...
      def self.usable?
        ...
        Heed.logger.debug format('wdm - load failed: %s:%s', $ERROR_INFO,
                                   $ERROR_POSITION * "\n")

        Heed.adapter_warn(BUNDLER_DECLARE_GEM)
        false
      end
    ...
    private
      ...
      def _configure(dir)
        require 'wdm'
        Heed.logger.debug 'wdm - starting...'
      ...
      def _process_event(dir, event)
        Heed.logger.debug "wdm - callback: #{event.inspect}"
