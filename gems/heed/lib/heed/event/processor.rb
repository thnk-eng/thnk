  ...
  module Event
    class Processor
      ...
      def loop_for(latency)
        ...
        Heed.logger.debug('Processing stopped')
      end
    ...
    private
      ...
      def _process_changes(event)
        ...
        ::Heed::Thread.rescue_and_log('_process_changes') do
          config.call(*result)
          exception_note = nil
        end
        Heed.logger.debug "Callback#{exception_note} took #{MonotonicTime.now - block_start} sec"
      end
