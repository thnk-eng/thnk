...
module Heed
  ...
  # If you don't want logging, set Heed.logger = ::Heed.new('/dev/null', level: ::Heed::UNKNOWN)
  ...
  class << self
    ...
    def adapter_warn(message)
      case ENV['HEED_GEM_ADAPTER_WARN_BEHAVIOR']&.to_sym || adapter_warn_behavior_callback(message)
        when :log
          logger.warn(message)
  ...
  private
    ...
    def default_logger
      level =
        case ENV['HEED_GEM_DEBUGGING'].to_s
          when /debug|2/i
            ::Logger::DEBUG
